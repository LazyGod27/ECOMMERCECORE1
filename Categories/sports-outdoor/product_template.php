...
$products_data = [
    ...
    foreach ($products_data as $id => &$p) {
        if ($id == 701) continue; // Skip already correct one
    ...
    $product = isset($products_data[$p_id]) ? $products_data[$p_id] : $products_data[701];
...
<div class="pv-right">
    ...
    <script>
        function addToCart() {
            ...
            window.location.href = `../../Content/add-to-cart.php?add_to_cart=1&product_name=${encodeURIComponent(fullName)}&price=${price}&image=${img}&quantity=${qty}&store=IMarket%20Sports%20Outdoor`;
        }
